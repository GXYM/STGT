...
class VisionTransformer(nn.Module):
    ...
    def position_embedding(self, x):
        ...
        L = x.size(1)
        ## Resizing positional embeddings in case they don't match
        if L != self.pos_embed.size(0):
            pos_embed = self.pos_embed.unsqueeze(0).transpose(1,2).contiguous()
            new_pos_embed = F.interpolate(pos_embed, size=(L), mode='nearest')
            new_pos_embed = new_pos_embed.transpose(1, 2).contiguous().squeeze(0)
            x = x + new_pos_embed
        else:
            x = x + self.pos_embed
    ...
    def forward_features(self, x, mask_ratio=0.0):
        ...
        _, L, C = x.shape
        x = torch.mean(x.view(B, T, L, C), dim=1, keepdim=False)
        z = torch.cat([x, gmhra_cls_token.permute(1, 0, 2)], dim=1)
        return z
